...
def build_ensemble() -> dict:
    ...
    return {
        "meta": {
            ...
            "note": "This IS the final combination: forecast.run applies these "
                    "finals as its last stage before the workbooks are written; "
                    "the constant-reliability engine value is retained beside "
                    "each final in the audit.",
        },
        "rows": rows,
    }
...
LENS_LABEL = {"anchor": "Anchor", "driver": "Driver", "ml": "ML", "market": "Market"}


def lens_mix(row: dict) -> str:
    """The weights string shown in the forecast table and re-checked by the guard."""
    voted = {item["lens"]: item for item in row["lenses"]}
    parts = [f"{LENS_LABEL[k]} {voted[k]['weight'] * 100:.0f}%" for k in voted]
    absent = [LENS_LABEL[k] for k in ("anchor", "driver", "ml", "market") if k not in voted]
    if absent:
        parts.append(f"{' / '.join(absent)} abstained")
    return " · ".join(parts)


def _engine_values() -> dict[tuple[str, str], float]:
    """Pre-ensemble engine values recorded by forecast.run in the audit."""
    if not AUDIT.exists():
        return {}
    ticker_label = {"LSE:HAS": "Hays"}
    out = {}
    for forecast in json.loads(AUDIT.read_text())["forecasts"]:
        tick = ticker_label.get(forecast["ticker"], forecast["ticker"])
        for metric in forecast["metrics"]:
            if "engine_value" in metric:
                out[(tick, metric["label"])] = metric["engine_value"]
    return out
...
def _section_html(payload: dict) -> str:
    ...
    for row in payload["rows"]:
        ...
        body.append(
            ...
            + (lambda ev: f'<td class="num">{_fmt(ev) if ev is not None else "—"}</td></tr>')(
                engine.get((row["ticker"], row["label"]))))
    ...
    <section id="ensemble">
      <div class="shell section-grid">
        ...
        <div>
          ...
          <p class="muted">This is the system's final combination: the last stage of
          <code>forecast.run</code> replaces each engine-mix value with this ensemble before the
          workbooks are written. Each lens is weighted by the inverse of its own backtested error for
          that exact metric: the ML gate score, the blinded LLM replay score, the anchor's
          ...
          another lens (the Hays driver), contributes nothing. The last column shows the
          constant-reliability engine value the ensemble replaced, kept in the audit beside each
          final. Generated by <code>forecast/ensemble.py</code>; machine-readable in
          ...
          <div class="table-wrap" role="region" aria-label="Four-lens ensemble" tabindex="0">
            <table>
              <thead><tr><th>Company</th><th>Metric</th><th>Anchor</th><th>Driver (blinded LLM)</th>
                <th>ML</th><th>Market</th><th>Ensemble (final)</th><th>Engine mix</th></tr></thead>
              <tbody>
                {chr(10).join(body)}
              </tbody>
            </table>
...
def inject(payload: dict) -> None:
    ...
    # Rewrite the 03 forecast table: value = the ensemble final forecast.run
    # submits, weights = the four-lens mix that produced it.
    start = html.index('aria-label="Forecast outputs"')
    end = html.index("</table>", start)
    table = html[start:end]
    for row in payload["rows"]:
        label = row["label"].replace("&", "&amp;")
        pattern = re.compile(
            rf'(<td>{re.escape(row["ticker"])}</td><td>{re.escape(label)}</td>'
            rf'<td class="num">)[^<]+(</td><td>[^<]*</td><td class="weights">)[^<]+(</td>)')
        replacement = rf'\g<1>{_fmt(row["final"])}\g<2>{lens_mix(row)}\g<3>'
        table, count = pattern.subn(replacement, table)
        if count != 1:
            raise RuntimeError(
                f"forecast table row not found for {row['ticker']} · {row['label']}")
    html = html[:start] + table + html[end:]

    # Copy edits and diagram callouts, applied from whichever variant is present
    # so re-running the injection is idempotent.
    flips = [
        ("Twelve final numbers, with the realized engine mix.",
         "Twelve final numbers, from the four-lens ensemble."),
        ("<th>Engine weights</th>",
         "<th>Lens weights</th>"),
        # original amber callout, main diagram
        ('<text class="d-warn" x="800" y="620">MEASURED SKILL SHOULD SET ENGINE RELIABILITY</text>\n'
         ...
         '<text class="d-good" x="800" y="620">MEASURED SKILL SETS THE FINAL FOUR-LENS ENSEMBLE (03B)</text>\n'
         '                <text class="d-mono" x="800" y="638">WEIGHT &#8733; 1/BACKTESTED ERROR &#183; APPLIED AS THE LAST STAGE OF FORECAST.RUN</text>'),
        # first-generation flip, main diagram
        ('<text class="d-good" x="800" y="620">MEASURED SKILL NOW SETS THE FOUR-LENS ENSEMBLE (03B)</text>\n'
         ...
         '<text class="d-good" x="800" y="620">MEASURED SKILL SETS THE FINAL FOUR-LENS ENSEMBLE (03B)</text>\n'
         '                <text class="d-mono" x="800" y="638">WEIGHT &#8733; 1/BACKTESTED ERROR &#183; APPLIED AS THE LAST STAGE OF FORECAST.RUN</text>'),
        # original amber callout, validation diagram
        ('<text class="d-warn" x="700" y="256">MEASURED SKILL SHOULD SET ENGINE RELIABILITY &#8212; NOT WIRED, RELIABILITY IS STILL A CONSTANT</text>',
         '<text class="d-good" x="700" y="256">MEASURED SKILL SETS THE FINAL FOUR-LENS ENSEMBLE WEIGHTS (03B) &#8212; APPLIED AS THE LAST STAGE OF FORECAST.RUN</text>'),
        # first-generation flip, validation diagram
